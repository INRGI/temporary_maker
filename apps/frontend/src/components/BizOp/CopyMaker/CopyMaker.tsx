...
import { CopyStyles, Preset } from "../../../types/bizop";
...
import { ResponseCopy } from "../../../types/bizop/copy-response";
...
const CopyMaker: React.FC<Props> = ({ preset }) => {
  ...
  const STORAGE_KEY = "bizop-last-copies";
  ...
  const makeCopies = async () => {
    try {
      ...
      const response = await axios.post(
        `/api/bizop/copy/make-multiple-copies`,
        {
          preset,
