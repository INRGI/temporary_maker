...
import { SubjectLine, Preset } from "../../../types/bizop";
...
const SubjectBlock: React.FC<Props> = ({ preset }) => {
  const [subject, setSubject] = useState<SubjectLine>({
    isSubjectLine:
      (preset?.subjectLine && preset.subjectLine?.isSubjectLine) || false,
    subjectLine:
      (preset?.subjectLine && preset.subjectLine?.subjectLine) || "Default",
  });
  ...
  const handleSave = () => {
    try {
      ...
      const updatedPresets = savedPresets.map((p: Preset) =>
        p.name === preset.name ? { ...p, subjectLine: subject } : p
      );
    ...
    <Container>
      <InputGroup>
        <StyledCheckbox
          ...
          onChange={() => handleChange("isSubjectLine", !subject.isSubjectLine)}
        />
        <Dropdown
          placeholder="Select Anti Spam"
          options={antiSpamOptions}
          selected={subject.subjectLine || "None"}
          onSelect={(value) => handleChange("subjectLine", value as string)}
        />
      </InputGroup>
      <InputGroup>
