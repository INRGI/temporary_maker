...
import {
  ...
  SaveButton,
} from "../PresetCreateModal/PresetCreateModal.styled";
import Loader from "../../Common/Loader";
import axios from "axios";
import { toastError, toastSuccess } from "../../../helpers/toastify";
import FloatingLabelInput from "../../Common/FloatingLabelInput/FloatingLabelInput";
import Dropdown from "../../Common/Dropdown/Dropdown";
import { Copy } from "../BuildSpaceAdLink/BuildSpaceAdLink.styled";
...
const AntiSpamModal: React.FC<AntiSpamModalProps> = ({ isOpen, onClose }) => {
  ...
  const runAntiSpam = async (option: string, text: string) => {
    try {
      setLoading(true);
      const response = await axios.post(`/api/bizop/copy/anti-spam`, {
        antiSpamType: option,
        html: text,
      });
